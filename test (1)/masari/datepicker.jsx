...
/* Inline trigger: shows the chosen date as a big pill, clicking opens the modal/sheet */
...
function DateModalDesktop({ start, end, present, onStart, onEnd, onPresent, onClose }) {
  ...
  const [active, setActive] = useState("start"); // which side the grid edits
  ...
  const clear = () => {
    setLocal({ start:{m:"",y:""}, end:{m:"",y:""}, present:false });
  };
      ...
      <div className="ds-sheet" onClick={e => e.stopPropagation()}>
        ...
        <div className="ds-body">
          <div className="ds-year-row">
            <button onClick={() => setLocal(l => ({...l, [active]: {...cur, y: String(parseInt(cur.y || thisYear)-1)}}))}><Icon name="chevronR"/></button>
            <div className="ds-year-val">{cur.y || thisYear}</div>
            <button onClick={() => setLocal(l => ({...l, [active]: {...cur, y: String(parseInt(cur.y || thisYear)+1)}}))}><Icon name="chevronL"/></button>
          </div>
